...
fn token_swap_state_account(client: &RpcClient, payer: &Keypair) -> Keypair {
    let token_swap_program_id = &spl_token_swap::id();
    let mint_rent =
        client.get_minimum_balance_for_rent_exemption(Account::LEN).unwrap();

    let token_swap_state_account = Keypair::new();

    let token_swap_account_instr =
        solana_program::system_instruction::create_account(
            &payer.pubkey(),
            &token_swap_state_account.pubkey(),
            mint_rent,
            Account::LEN as u64,
    ...
    match client.send_and_confirm_transaction(&mint_transaction) {
        Ok(sig) => {
            println!("Create token swap account - View your transaction on the Solana Explorer at:\nhttps://explorer.solana.com/tx/{sig}?cluster=devnet");
        }
        Err(e) => println!("Error: {e}"),
    };
...
fn swap_pool_authority(token_swap_state_account: &Pubkey) -> Pubkey {
    let token_swap_program_id = &spl_token_swap::id();

    Pubkey::find_program_address(
        &[token_swap_state_account.as_ref()],
        token_swap_program_id,
    )
    .0
}
...
fn main() -> web3::Result<()> {
    ...
    let token_swap_state_account = token_swap_state_account(&client, &payer);
    let owner = swap_pool_authority(&token_swap_state_account.pubkey());
    ...
    create_token_account(&client, &payer, &token_account_a, &mint_account);
    create_token_account(&client, &payer, &token_account_b, &mint_account);
